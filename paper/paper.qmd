...
Fully Homomorphic Encryption (FHE) is often described as the "holy grail" of cryptography. It enables arbitrary computation on ciphertexts. Formally, given encryptions $E(x)$ and $E(y)$, FHE allows the computation of $E(x + y)$ and $E(x \cdot y)$ without ever decrypting the inputs. Since any computable function can be represented as a circuit of addition and multiplication gates, FHE allows a server to compute any function $f$ on encrypted data, yielding an encrypted result $E(f(x))$.
