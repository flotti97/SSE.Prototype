...
The encrypted index, referred to as the TSet, is implemented as a dictionary [@Cash2013HighlyScalableSS]. For a keyword $w$ appearing in a list of documents $D_w = (id_0, id_1, \dots, id_{r-1})$, the client generates two keys derived from the master key $K$:
...
To perform a search for keyword $w$, the protocol proceeds as follows [@Cash2013HighlyScalableSS]:
...
The scheme relies on computations in a cyclic group $\mathbb{G}$ of prime order $p$ with generator $g$. In practice, $p$ should be a large safe prime (a prime of the form $p = 2q + 1$ where $q$ is also prime) to ensure the discrete logarithm problem remains computationally hard [@Cash2013HighlyScalableSS]. We use $\mathbb{Z}_p^*$ as the underlying group with modular arithmetic operations.

#### Data Structures {#sec-oxt-data}

The server stores two main data structures [@Cash2013HighlyScalableSS]:
...
To perform a conjunctive search for $w_1 \land w_2$ [@Cash2013HighlyScalableSS]:
...
**Leakage Profile:** The OXT protocol reveals the access pattern of the pivot keyword ($w_1$) and the result pattern of the intersection ($w_1 \land w_2$) [@Cash2013HighlyScalableSS]. Crucially, it hides the access pattern of the non-pivot keywords ($w_2$). The server learns the *conditional* probability: "Given document $D$ contains $w_1$, does it also contain $w_2$?" but does not learn if a document contains $w_2$ if it does not contain $w_1$.
